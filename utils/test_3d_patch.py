...
def var_all_case_pan(model, num_classes, patch_size=(96, 96, 96), stride_xy=16, stride_z=4):
    with open('../data/pancreas/test.list', 'r') as f:
        case_ids = [line.strip() for line in f if line.strip()]
        image_list = ["../NIH_UPCoL/data/data{}".format(case_id) for case_id in case_ids]
...
def var_all_case_msd(model, num_classes, patch_size=(96, 96, 96), stride_xy=16, stride_z=4):
    with open('../data/MSD/val.txt', 'r') as f:
        case_ids = [line.strip() for line in f if line.strip()]
        image_list = ["../data/MSD/{}".format(case_id) for case_id in case_ids]
    loader = tqdm(image_list)
    total_dice = 0.0
    for image_path in loader:
        h5f = h5py.File(image_path, 'r')
...
def test_all_case(model, image_list, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, save_result=True, test_save_path=None, preproc_fn=None, metric_detail=0, nms=0, print_result=1):
    dc_list = []
    jc_list = []
    hd95_list = []
    asd_list = []
    ...
    print('std is {}'.format(std_metric))
